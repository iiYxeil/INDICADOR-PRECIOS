...
def recoge_btc(get):
    ...
    fuentes = [
        ...
        ("sma",   "mayer-multiple",      ["200 Day Moving Average",
                                          "200-Day Moving Average",
                                          "200D Moving Average"],             1000, 500000),
    ]
    for clave, ruta, etiquetas, lo, hi in fuentes:
        try:
            ...
            if clave == "puell" and "precio" not in d:
                pr = _tras_etiqueta(txt, ["BTC Daily Price"], 1000, 1000000)
                if pr is not None:
                    d["precio"] = pr
        except Exception as e:
